...
fn seeded() -> (Index, i64, i64) {
    ...
    let repo_id = index.upsert_repository(&repo("my-repo", "/Users/you/code/my-repo")).unwrap();
    let session_id = index.upsert_session(&session(ToolId::ClaudeCode, "sess-1", Some(repo_id))).unwrap();
    index
        .replace_messages(
            session_id,
            &[
                msg(0, Role::User, "the nightly job dies with ECONNRESET"),
                msg(1, Role::Assistant, "the pool keeps sockets longer than the gateway does"),
                msg(2, Role::ToolResult, "logs/nightly.log:118: Error: read ECONNRESET"),
            ],
        )
        .unwrap();
...
fn a_fresh_index_is_empty_but_usable() {
    ...
    assert!(index.list_sessions(&SessionQuery::default()).unwrap().is_empty());
    assert!(index.search(&SearchQuery { text: "anything".into(), ..Default::default() }).unwrap().is_empty());
}

#[test]
fn repositories_are_keyed_on_identity_not_on_row_order() {
    let mut index = Index::open_in_memory().unwrap();
    let first = index.upsert_repository(&repo("my-repo", "/Users/you/code/my-repo")).unwrap();
    ...
    assert_eq!(first, second, "the same identity must update, not duplicate");
...
fn sessions_are_keyed_on_tool_and_native_id() {
    let mut index = Index::open_in_memory().unwrap();
    let a = index.upsert_session(&session(ToolId::ClaudeCode, "same-id", None)).unwrap();
    let b = index.upsert_session(&session(ToolId::ClaudeCode, "same-id", None)).unwrap();
    let c = index.upsert_session(&session(ToolId::Codex, "same-id", None)).unwrap();
...
fn a_session_round_trips_through_the_index() {
    ...
    assert_eq!(stored.message_count, 3, "the count follows the messages that were stored");
    ...
    assert_eq!(messages[2].content, "logs/nightly.log:118: Error: read ECONNRESET");
}

#[test]
fn full_text_search_finds_a_term_across_a_session() {
    let (index, _, session_id) = seeded();
    let hits = index.search(&SearchQuery { text: "ECONNRESET".into(), ..Default::default() }).unwrap();
    ...
    assert!(
        hits.iter().any(|h| h.repo_display.as_deref() == Some("my-repo")),
...
fn search_is_case_insensitive() {
    ...
    let lower = index.search(&SearchQuery { text: "econnreset".into(), ..Default::default() }).unwrap();
...
fn search_finds_japanese_without_spaces() {
    ...
    let mut index = Index::open_in_memory().unwrap();
    let session_id = index.upsert_session(&session(ToolId::Codex, "jp", None)).unwrap();
    index
        .replace_messages(
            session_id,
            &[msg(0, Role::User, "接続がリセットされました。リトライ間隔を2秒にします。")],
        )
        .unwrap();

    for term in ["リセット", "リトライ間隔", "接続"] {
        let hits = index.search(&SearchQuery { text: term.into(), ..Default::default() }).unwrap();
...
fn search_can_be_narrowed_to_a_repository_a_session_or_a_tool() {
    ...
    let other_repo = index.upsert_repository(&repo("other", "/Users/you/code/other")).unwrap();
    let other = index.upsert_session(&session(ToolId::Codex, "sess-2", Some(other_repo))).unwrap();
    index.replace_messages(other, &[msg(0, Role::User, "another ECONNRESET report")]).unwrap();

    let all = index.search(&SearchQuery { text: "ECONNRESET".into(), ..Default::default() }).unwrap();
    assert_eq!(all.len(), 3);

    let by_repo = index
        .search(&SearchQuery { text: "ECONNRESET".into(), repo_id: Some(repo_id), ..Default::default() })
        .unwrap();
    assert_eq!(by_repo.len(), 2);

    let by_session = index
        .search(&SearchQuery { text: "ECONNRESET".into(), session_id: Some(other), ..Default::default() })
        .unwrap();
    assert_eq!(by_session.len(), 1);

    let by_tool = index
        .search(&SearchQuery { text: "ECONNRESET".into(), tool: Some(ToolId::Codex), ..Default::default() })
        .unwrap();
...
fn a_phrase_search_is_not_an_and_search() {
    ...
    let (index, _, _) = seeded();
    let phrase = index
        .search(&SearchQuery { text: "\"keeps sockets\"".into(), ..Default::default() })
        .unwrap();
    assert_eq!(phrase.len(), 1);

    let nonsense = index
        .search(&SearchQuery { text: "\"sockets keeps\"".into(), ..Default::default() })
        .unwrap();
...
fn search_input_cannot_break_the_query() {
    ...
    let (index, _, _) = seeded();
    for text in ["ECONNRESET*", "a\"b", "(", "NEAR/", "'; drop table sessions; --", ""] {
        let result = index.search(&SearchQuery { text: text.into(), ..Default::default() });
        assert!(result.is_ok(), "query {text:?} failed: {:?}", result.err());
    }
    assert_eq!(index.stats().unwrap().sessions, 1, "the table is still there");
}

#[test]
fn replacing_messages_replaces_what_is_searchable() {
    let (mut index, _, session_id) = seeded();
    index.replace_messages(session_id, &[msg(0, Role::User, "completely different content")]).unwrap();

    assert!(index.search(&SearchQuery { text: "ECONNRESET".into(), ..Default::default() }).unwrap().is_empty());
    assert_eq!(
        index.search(&SearchQuery { text: "completely".into(), ..Default::default() }).unwrap().len(),
        1
    );
...
fn session_listing_filters_and_orders() {
    ...
    assert_eq!(ids, vec!["recent", "elsewhere", "old"], "most recently touched first (FR-4.4)");

    let in_repo = index
        .list_sessions(&SessionQuery { repo_id: Some(repo_id), ..Default::default() })
        .unwrap();
    assert_eq!(in_repo.len(), 2);

    let codex_only = index
        .list_sessions(&SessionQuery { tool: Some(ToolId::Codex), ..Default::default() })
        .unwrap();
    assert_eq!(codex_only.len(), 2);

    let since = index
        .list_sessions(&SessionQuery { since: Some(5_000), ..Default::default() })
        .unwrap();
    assert_eq!(since.len(), 2);

    let busiest = index
        .list_sessions(&SessionQuery { order: SessionOrder::MessagesDesc, ..Default::default() })
        .unwrap();
    assert_eq!(busiest[0].native_id, "old");

    let limited = index
        .list_sessions(&SessionQuery { limit: Some(1), ..Default::default() })
        .unwrap();
...
fn an_archived_session_keeps_its_transcript() {
    ...
    assert_eq!(
        index.search(&SearchQuery { text: "ECONNRESET".into(), ..Default::default() }).unwrap().len(),
        2,
...
fn the_index_survives_being_closed_and_reopened() {
...

    {
        ...
        let session_id = index.upsert_session(&session(ToolId::ClaudeCode, "persist", None)).unwrap();
        index.replace_messages(session_id, &[msg(0, Role::User, "durable ECONNRESET")]).unwrap();
    }

    let index = Index::open(&path).unwrap();
    assert_eq!(index.stats().unwrap().sessions, 1);
    assert_eq!(
        index.search(&SearchQuery { text: "durable".into(), ..Default::default() }).unwrap().len(),
        1
    );
}
