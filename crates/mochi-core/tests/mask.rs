...
fn kinds(input: &str) -> Vec<&'static str> {
    let mut k: Vec<_> = Masker::new().mask(input).findings.into_iter().map(|f| f.kind).collect();
    k.sort_unstable();
    k.dedup();
    k
}
...
fn openai_style_keys_are_masked() {
    ...
    assert!(out.contains("OPENAI_API_KEY="), "the key name is useful context: {out}");
...
fn github_tokens_are_masked() {
    for (text, kind) in [
        ...
        ("github_pat_EXAMPLEEXAMPLE_EXAMPLEEXAMPLEEXAMPLE", "github_pat"),
    ] {
        let out = mask(text);
...
fn bearer_headers_are_masked_but_the_scheme_stays() {
    ...
    let out = mask(text);
    assert!(out.contains("Bearer"), "the reader should still see what kind of header it was: {out}");
    assert!(!out.contains("EXAMPLEtokenEXAMPLE"), "got {out}");
    assert!(out.contains("https://api.internal/v1"), "the url is not a secret: {out}");
}
...
fn private_key_blocks_are_masked_whole() {
    ...
    assert!(!out.contains("EXAMPLEbody"), "the body must go, not just the header: {out}");
...
fn credentials_inside_urls_are_masked() {
    ...
    assert!(out.contains("db.internal"), "the host is not the secret: {out}");
}
...
fn secret_looking_assignments_are_masked() {
    ...
    for text in [
        ...
        let out = mask(text);
        assert!(!out.contains("EXAMPLEEXAMPLE") && !out.contains("Zm9vYmFyEXAMPLE"), "{text} -> {out}");
    }
}
...
fn ordinary_text_is_left_completely_alone() {
    ...
    for text in [
        ...
        assert!(masked.is_clean(), "false positive on {text:?}: {:?}", masked.findings);
...
fn findings_point_at_the_input() {
    ...
    assert!(text[f.start..f.end].starts_with("sk-"), "span was {:?}", &text[f.start..f.end]);
}
...
fn multibyte_text_around_a_secret_is_preserved() {
    ...
    let text = "設定ファイルの中身: OPENAI_API_KEY=sk-EXAMPLEEXAMPLEEXAMPLEEXAMPLEEXAMPLEEX でした。";
    let out = mask(text);
...
fn several_secrets_in_one_blob_all_go() {
    ...
    assert_eq!(result.text.lines().count(), 3, "line structure must survive: {}", result.text);
}
